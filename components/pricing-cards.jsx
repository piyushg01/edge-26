...
const PricingCards = () => {
  ...
  return (
    ...
    >
      ...
      <TabsContent
        value="Vitian"
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 w-full md:gap-8 max-w-5xl mx-auto pt-6"
      >
        {PLANS.map((plan) => (
          <Card
            key={plan.name}
            className={cn(
              ...
              plan.highlight && "border-2 border-yellow-600"
            )}
          >
            <CardHeader
              className={cn(
            ...
            >
              <CardTitle
                className={cn(
                  !plan.highlight && "text-muted-foreground",
                  "text-lg font-medium"
                )}
              >
                {plan.name}
